...
class wholeManagementCase(unittest.TestCase):
    ...
    def test_pen_register_landlord(self):
        # parent_element = self.driver.find_element(By.XPATH, '//wujie-app')
        # 访问 shadowRoot 属性
        # shadow_root = self.driver.execute_script('return arguments[0].shadowRoot', parent_element)
        time.sleep(5)
        ActionChains(self.driver).move_by_offset(1749,100).click().perform()
        time.sleep(2)
        ActionChains(self.driver).move_by_offset(450,250).click().perform()
