...
def main(args=None):
    ...
    if not options.test_targets:
        raise ValueError("Must provide target test folder")
    # 初始化全局变量
    init()
    # 设置全局环境变量
    ...
    # 设置out_put_folder
    out_put_base_folder = options.report_file
    ...
    # 从默认文件夹tests开始查找测试用例
    case_finder = DiscoverTestCases(options.test_targets)
    # 查找测试模块并导入
    ...
    if raw_test_suites:
        ...
        raise ValueError("No case to run")
    ...
    generate_html_report(get_config('out_put_folder'), cases_run_success, cases_run_fail, cases_encounter_error,
                         skipped_cases, time.time(), platform.system(), False)
    log.info("\n{pre_fix} Generate report done, please check report.html from '{report}'. {pre_fix} ".format(
        pre_fix='---' * 10, report=get_config('out_put_folder')))


if __name__ == "__main__":
    # main("-env prod -i smoke -t ./tests -r ./output")
    main()